...
class DesConnector @Inject() (
  ...
  private val authorizationToken: String =
    configuration.underlying.getString("microservice.services.des.authorizationToken")
  ...
  def sendCardPaymentsNotification(chargeRefNotificationDesRequest: ChargeRefNotificationDesRequest): Future[Unit] = {
    ...
    logger.debug(
      s"Calling des api 1541 for chargeRefNotificationDesRequest ${chargeRefNotificationDesRequest.toString}"
    )
